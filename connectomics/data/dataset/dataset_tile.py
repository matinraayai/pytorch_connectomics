...
from ..utils import crop_volume, relabel,seg_widen_border, tile_to_volume
...
class TileDataset(torch.utils.data.Dataset):
    ...
    def update_chunk(self, do_load=True):
        ...
        x0, x1 = np.floor(np.array([xid, xid + self.chunk_step]) /
                          (self.chunk_num[2] + self.chunk_step - 1) *
                          self.json_size[2]).astype(int)
        y0, y1 = np.floor(np.array([yid, yid + self.chunk_step]) /
                          (self.chunk_num[1] + self.chunk_step - 1) *
                          self.json_size[1]).astype(int)
        z0, z1 = np.floor(np.array([zid, zid + self.chunk_step]) /
                          (self.chunk_num[0] + self.chunk_step - 1) *
                          self.json_size[0]).astype(int)
    ...
    def loadchunk(self):
        ...
        if self.json_valid is not None:
            valid_mask = [tile_to_volume(self.json_valid['image'], coord_p, self.coord_m, \
